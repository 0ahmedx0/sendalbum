...
async def process_channel(client: Client, source_invite: str, dest_invite: str):
    ...
    # استبعاد الفيديوهات اللي تحتوي على thumbnail
    filtered_out_with_thumb = [
        m for m in all_messages
        if not m.media_group_id and (m.video and m.video.thumbs)
    ]

    # الاحتفاظ فقط بالصور + الفيديوهات بدون thumbnail
    non_album_messages = [
        m for m in all_messages
        if not m.media_group_id and (
            m.photo or (m.video and not m.video.thumbs)
        )
    ]

    print(f"📛 تم استبعاد {len(filtered_out_with_thumb)} فيديو يحتوي على صورة مصغّرة (thumbnail).")
    ...
    print(f"📛 عدد الفيديوهات المستبعدة بسبب وجود thumbnail: {len(filtered_out_with_thumb)}")
